...
2. 阅读 [课程计划表](docs/course-plan.md)，确认三级小节、交付物和运行验证。
3. 阅读 [K1 / Muse Pi Pro 开发板选型说明](boards/k1/board-selection.md)，确认课程目标板、LicheePi 4A 临时验证板和 QEMU 辅助环境的边界。
4. 从 `chapters/ch01_ruyisdk_qemu` 开始，完成环境准备、系统启动和登录。
5. 继续完成 `chapters/ch02_toolchain_template`，建立工具链、Makefile 和最小工程能力。
6. 进入 `chapters/ch03_gpio_interaction`，完成 GPIO Blink、Button 和状态机交互实验。
7. 按章节顺序推进到 ch08，形成联网信息采集终端主线作品。
8. 根据兴趣选择 ch09 到 ch11 的扩展方向。
